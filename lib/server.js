...
function ping(timeout) {
  ...
  for(key in this.hosts)
    batch.push((function(host) {
      ...
      connection.setTimeout(timeout, error.bind(connection, host));
      connection.on("error", error.bind(connection, host));
      connection.on("connect", success.bind(connection, host));
