...
Server.prototype.getHost = function(host) {
  ...
  for(var key in this.hosts)
    if(this.hosts[key].regex.test(host))
...
function ping(timeout) {
  ...
  for(key in this.hosts)
    batch.push((function(host) {
      ...
      connection.setTimeout(timeout, error.bind(this, connection, host));
      connection.on("error", error.bind(this, connection, host));
      connection.on("connect", success.bind(this, connection, host));
    }).bind(this, this.hosts[key]));
  batch.end();

  function error(socket, host) {
    socket.destroy();
    if(host.path) fs.unlink(host.path);
    delete this.hosts[host.key];
  }
  function success(socket, host) {
    socket.end();
  }
}
