 ...
 * @return {net.Server}
 */
function patch(server, waitForAddress) {
  ...
  server.listen = listen.bind(server, server.listen.bind(server));
...
function listen(_listen, port, domain, cb) {
  _listen(0, "localhost", onListening.bind(this, port, domain, cb));
  return this;
}
...
function onListening(port, domain, cb) {
  ...
  self._host = net.connect(port, "localhost")
    .once("connect", function() {
      ...
      var buf = [
              new Buffer([0xBA, 0xDA, 0x55, 0xC0, 0xFF, 0xEE])
            , new Buffer(4)
            , new Buffer(domain, "utf8")
            ];
      buf[1].writeUInt32BE(self.address().port, 0);

      this.write(Buffer.concat(buf));
    })
...
function requestHost(query, cb) {
  query.id = query.id || uuid.v4();
  this.once(query.id, cb);
  this._host.write(JSON.stringify(query));
}
...
function getAddress(socket, cb) {
  var port = socket._getpeername().port;
  socket._peername = {};
  socket._sockname = {};

  requestHost.call(this, { request: "address", port: port }, function(data) {
